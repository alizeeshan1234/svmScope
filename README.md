...
| `SVMSCOPE_REPLAY_WINDOW_DAYS` | How far back a replay at slot reaches, by block time (default 30, the recorder's retention). Older slots are refused with a plain message, so the product promises only what its own archive keeps, whatever a third-party stream still holds. |
